...
const { SQSClient, SendMessageCommand, ReceiveMessageCommand, DeleteMessageCommand, SendMessageBatchCommand } = require('@aws-sdk/client-sqs');
...
async function send_sqs_message(queue_name, message, options) {
    const params = {...options};
    params.QueueUrl = queue_name.startsWith('https:') ? queue_name : 
...
async function send_sqs_message_batch(queue_name, messages, options) {
    ...
    for (const message of messages) { 
        ...
        if (queue_name.endsWith('.fifo')) {
            if (!entry.MessageGroupId) entry.MessageGroupId = queue_name;
            if (!entry.MessageDeduplicationId) entry.MessageDeduplicationId = get_check_sum(entry.MessageBody);
        }
        Entries.push(entry);
